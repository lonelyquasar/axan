...
// Function Description:
// - Returns the path to either conhost.exe or the side-by-side console host, depending on whether this
//   module is building with Windows and the side-by-side host could be found.
//   axan: the side-by-side host is axan-console.exe (renamed from OpenConsole.exe, #424).
// Return Value:
...
static wchar_t* _ConsoleHostPath()
{
    ...
    static auto consoleHostPath = []() {
        ...
        modulePath.replace_filename(L"axan-console.exe");
        if (!std::filesystem::exists(modulePath))
        {
            ...
            modulePath.append(L"axan-console.exe");
        }
        if (!std::filesystem::exists(modulePath))
        {
