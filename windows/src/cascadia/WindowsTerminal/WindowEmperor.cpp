...
{
    // axan #445: breadcrumb every attempt outcome. The morning-of-2026-07-02 failure mode
    // was an instance that hung pre-window while holding this mutex: every retry below spun
    // for ~30s, fell out of the loop, and exited pretending it had handed off. Nothing
    // anywhere recorded that. These logs make each of the three exits distinguishable.
    ...
    for (DWORD sleep = 50; sleep < 10000; sleep += sleep / 2)
    {
        ...
        if (mutex)
        {
            Axan::Log::Info("Bootstrap", "single-instance mutex acquired; this process is the instance owner");
            return mutex;
        }
        ...
        if (const auto hwnd = FindWindowW(className, nullptr))
        {
            ...
            {
                Axan::Log::Info("Bootstrap", "handed off commandline to existing instance", { { "ownerPid", std::to_string(processId) } });
                return {};
            }
            Axan::Log::Warn("Bootstrap", "found owner window but WM_COPYDATA handoff failed; retrying", { { "ownerPid", std::to_string(processId) }, { "gle", std::to_string(GetLastError()) } });
        }
        else if (!loggedWaiting)
        {
            loggedWaiting = true;
            Axan::Log::Warn("Bootstrap", "instance mutex is held but the owner has no window yet; waiting up to ~30s for a handoff target");
        }

        Sleep(sleep);
    }

    // The owner held the mutex for the whole ~30s without ever creating a window — it is
    // almost certainly hung during startup (#445). Returning empty makes the caller exit
    // as if the commandline was handed off, i.e. silently. Log it loudly first.
    Axan::Log::Error("Bootstrap", "gave up waiting for the instance owner's window after ~30s; owner is likely hung pre-window — exiting without doing anything (#445)");
    return {};
}
...
void WindowEmperor::HandleCommandlineArgs(int nCmdShow)
{
    ...
    // axan, not "Windows Terminal": this string names the single-instance mutex and the
    // message-window class. The stable Microsoft Windows Terminal is also a Release build,
    // and for a packaged app the class is just <brand> + <user SID hash> (the path hash is
    // added only when !IsPackaged()). Sharing the "Windows Terminal" brand here makes axan
    // hand its commandline off to a running stock WT instead of opening its own window.
    windowClassName.append(L"axan");
    ...
    Axan::Log::Info("Bootstrap", "constructing XAML application object");
    _app = winrt::TerminalApp::App{};
    Axan::Log::Info("Bootstrap", "XAML application constructed; loading settings");
    _app.Logic().ReloadSettings();

    _createMessageWindow(windowClassName.c_str());
    // axan #445: from here on, retries of a hung launch can at least find the handoff target.
    Axan::Log::Info("Bootstrap", "message window created; instance can now accept handoffs");
    _setupGlobalHotkeys();
