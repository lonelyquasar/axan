...
void AppHost::_AppTitleChanged(const winrt::Windows::Foundation::IInspectable& /*sender*/, const winrt::Windows::Foundation::IInspectable& /*args*/)
{
    ...
    // axan: also render the title inside the custom titlebar (centered session
    // label + terminal title), when we're drawing our own frame.
    if (_useNonClientArea)
    {
        auto nonClientWindow{ static_cast<NonClientIslandWindow*>(_window.get()) };
        nonClientWindow->SetTitlebarTitleText(_windowLogic.TitlebarPrimaryText(), _windowLogic.TitlebarSecondaryText());
    }
}
...
void AppHost::_UpdateTitleBarContent(const winrt::Windows::Foundation::IInspectable&, const winrt::Windows::UI::Xaml::UIElement& arg)
{
    if (_useNonClientArea)
    {
        ...
        // axan: seed the centered title text so it's there at first layout,
        // before the first TitleChanged fires.
        nonClientWindow->SetTitlebarTitleText(_windowLogic.TitlebarPrimaryText(), _windowLogic.TitlebarSecondaryText());
    }
