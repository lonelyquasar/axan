...
int __stdcall wWinMain(HINSTANCE, HINSTANCE, LPWSTR, int nCmdShow)
{
    ...
    // axan #445: earliest possible file-log breadcrumb. AxanLog needs COM (ApplicationData),
    // so this can't move above init_apartment; a launch that never logs even this line hung
    // in the loader / pre-apartment code (Defender cold-scanning the loose Debug layout is
    // the leading suspect there).
    Axan::Log::Info("Bootstrap", "wWinMain: apartment initialized, creating WindowEmperor");
