...
void NonClientIslandWindow::Initialize()
{
    ...
    // axan: replay any title text that arrived before _titlebar existed.
    if (!_titlebarPrimaryText.empty() || !_titlebarSecondaryText.empty())
    {
        _titlebar.SetTitleText(_titlebarPrimaryText, _titlebarSecondaryText);
    }
}

// Method Description:
// - axan: push the active session's name (+ terminal title) into the
//   TitlebarControl's centered title text. Called by AppHost whenever the
//   app's TitleChanged fires. Cached so a pre-Initialize call isn't lost.
void NonClientIslandWindow::SetTitlebarTitleText(const winrt::hstring& primary, const winrt::hstring& secondary)
{
    _titlebarPrimaryText = primary;
    _titlebarSecondaryText = secondary;
    if (_titlebar)
    {
        _titlebar.SetTitleText(primary, secondary);
    }
}
