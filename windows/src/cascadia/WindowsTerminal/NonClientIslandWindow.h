...
public:
    ...
    // axan: forward the active session's name (+ terminal title) to the
    // TitlebarControl's centered title text. Values are cached so a call that
    // arrives before Initialize() creates _titlebar isn't lost.
    void SetTitlebarTitleText(const winrt::hstring& primary, const winrt::hstring& secondary);
...
private:
    ...
    // axan: last title text pushed via SetTitlebarTitleText, replayed onto
    // _titlebar at the end of Initialize().
    winrt::hstring _titlebarPrimaryText;
    winrt::hstring _titlebarSecondaryText;
