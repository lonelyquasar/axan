...
namespace winrt::TerminalApp::implementation
{
    ...
    void TerminalPage::Create()
    {
        ...
        // axan M2: the sidebar TreeView is the tree of shell sessions. Mirror the live
        // session list (_tabs) into it, and route node activation through the existing
        ...
        SessionTree().ContextRequested({ get_weak(), &TerminalPage::_OnSessionTreeContextRequested });
        // axan M13: when the sidebar's resolved theme changes, repaint node labels with the
        // theme-correct default text color (fires after ActualTheme actually settles).
        if (const auto sidebar{ SessionSidebar() })
        {
            sidebar.ActualThemeChanged([weak = get_weak()](auto&&, auto&&) {
                if (auto p{ weak.get() }) { p->_RefreshNodeLabelBrushes(); }
            });
        }
        // axan M14/D19: the curated startup tree is now authored in the Settings "Startup
        // sessions" page (GlobalSettings().StartupSessions()), so the sidebar Save/Import
        // actions are retired.

        // axan M8: sidebar collapse states. The chevron cycles, the collapsed strip expands.
        // The minimized state shows a flat icon list (MinimizedList) backed by this observable
        // vector — rebuilt from the tree whenever nodes change while minimized (see
        // _OnSessionsCollectionChanged). Restore the persisted collapse state from the
        // previous run (ApplicationState) and apply it.
        SidebarCollapseButton().Click({ get_weak(), &TerminalPage::_OnSidebarCollapseClick });
        SidebarExpandStrip().Click({ get_weak(), &TerminalPage::_OnSidebarExpandStripClick });
        // axan M12: the draggable divider that resizes the expanded sidebar.
        SidebarResizeThumb().DragDelta({ get_weak(), &TerminalPage::_OnSidebarResizeDelta });
        SidebarResizeThumb().DragCompleted({ get_weak(), &TerminalPage::_OnSidebarResizeCompleted });
        // axan M12 fix: the Thumb resizes on drag but never advertised it — the pointer stayed an
        // arrow over the seam (unlike the window's own edges, which flip to the resize cursor). XAML
        // Islands has no per-element Cursor in system XAML, so drive it with Win32 SetCursor on hover.
        // WM_SETCURSOR resets to the arrow on every move, so we (re)assert on both enter AND move; the
        // last writer per WM_MOUSEMOVE wins, so it sticks while the pointer is over (or dragging) the seam.
        SidebarResizeThumb().PointerEntered({ get_weak(), &TerminalPage::_OnSidebarResizeThumbHover });
        SidebarResizeThumb().PointerMoved({ get_weak(), &TerminalPage::_OnSidebarResizeThumbHover });
        SidebarResizeThumb().PointerExited({ get_weak(), &TerminalPage::_OnSidebarResizeThumbLeave });
        _minimizedItems = winrt::single_threaded_observable_vector<winrt::TerminalApp::SessionNodeViewModel>();
        MinimizedList().ItemsSource(_minimizedItems);
        _sidebarState = ApplicationState::SharedInstance().SidebarState();
        _ApplySidebarState(_sidebarState);
    ...
    void TerminalPage::_CreateNewTabFlyout()
    {
        ...
        // add static items
        _AppendCommonMenuItems(newTabFlyout);
    ...
    // Method Description:
    // - axan: append the static app-level tail — settings, command palette, about —
    //   to the given flyout. Extracted from _CreateNewTabFlyout so the titlebar app
    //   menu (_CreateAppMenuFlyout) shows the identical items and the two can't drift.
    void TerminalPage::_AppendCommonMenuItems(const WUX::Controls::MenuFlyout& flyout)
    {
        ...
        flyout.Items().Append(commandPaletteFlyout);
        ...
        flyout.Items().Append(aboutFlyout);
    }
                                                                                        ...
                                                                                        const bool inheritCursor)
    {
        ...
        {
            // axan M10: shell-integration injection. Rewrite the spawn so the shell emits
            // OSC 9;9 (cwd) + OSC 133 marks each prompt — the emit half of the M5 contract,
            // without which a `cd` never moves a session label. PowerShell/WSL get an argv
            // rewrite; cmd gets a %PROMPT% environment var. A recognized-shell-launched-plainly
            // gate keeps us from clobbering a user's custom -Command/one-shot spawn, and the
            // whole thing degrades to the unmodified spawn on any failure (it's a nicety).
            auto axanCommandline = settings.Commandline();
            {
                const auto integ = Axan::ComputeShellIntegration(std::wstring_view{ axanCommandline }, true);
                if (integ.applied)
                {
                    axanCommandline = winrt::hstring{ integ.commandline };
                    if (integ.promptEnv)
                    {
                        auto env = settings.EnvironmentVariables();
                        if (env == nullptr)
                        {
                            env = winrt::single_threaded_map<winrt::hstring, winrt::hstring>();
                            settings.EnvironmentVariables(env);
                        }
                        env.Insert(L"PROMPT", winrt::hstring{ *integ.promptEnv });
                    }
                }
            }
            ...
            valueSet = TerminalConnection::ConptyConnection::CreateSettings(axanCommandline,
                                                                            newWorkingDirectory,
...
#undef ON_ALL_ACTIONS
    ...
    void TerminalPage::_RegisterTerminalEvents(TermControl term)
    {
        ...
        // axan M11: input-safety guard — host the Ctrl+C confirm dialog.
        term.ShowCtrlCInputGuard({ this, &TerminalPage::_ShowCtrlCInputGuardHandler });
    ...
    // Method Description:
    // - axan M11: input-safety guard. The focused control reported a Ctrl+C that
    //   would discard unsubmitted input. Confirm with the user before sending the
    //   interrupt; the dialog defaults to Cancel so a reflexive Enter preserves the
    //   typed line. The dialog is built programmatically (no XAML/resw) — ShowDialog
    //   parents it and sets its XamlRoot.
    // Arguments:
    // - sender: the TermControl that withheld the interrupt
    safe_void_coroutine TerminalPage::_ShowCtrlCInputGuardHandler(const IInspectable sender, const IInspectable /*eventArgs*/)
    {
        ...
        if (const auto presenter{ _dialogPresenter.get() })
        {
            ...
            dialog.Title(winrt::box_value(winrt::hstring{ L"Unsubmitted input" }));
            dialog.Content(winrt::box_value(winrt::hstring{ L"You have typed input that hasn't been submitted yet. Sending an interrupt (Ctrl+C) will discard the current line. Send it anyway?" }));
            dialog.PrimaryButtonText(winrt::hstring{ L"Send Ctrl+C" });
            dialog.CloseButtonText(winrt::hstring{ L"Cancel" });
            // Default to the safe choice: a stray Enter cancels rather than interrupts.
        ...
        if (confirmed)
        {
            control.ConfirmPendingInterrupt();
        }
        else
        {
            control.CancelPendingInterrupt();
        }
    }
    ...
    void TerminalPage::_RefreshUIForSettingsReload()
    {
        ...
        _UpdateTabWidthMode();
        _CreateNewTabFlyout();
        // axan: rebuild the titlebar app menu too — its accelerator hints come from
        // the (possibly rebound) action map.
        _CreateAppMenuFlyout();
    ...
    void TerminalPage::_updateThemeColors()
    {
        ...
        // axan M12 (B.11): theme the session sidebar to match WT's chrome.
        //
        // Two moves: (1) propagate the requested light/dark/default theme onto the sidebar
        // root so the tree's labels and auto-glyph FontIcons recolor with the theme
        // foreground (the B.7 "icon recolor reads the active theme foreground" requirement);
        // (2) paint the sidebar background with `bgColor` — the same effective color the
        // titlebar/tab-row resolved to above, including any custom theme.tabRow.background
        // or accent the user set — so the sidebar reads as one surface with the tab strip
        // instead of a differently-shaded panel. We use a *solid* brush built from bgColor
        // rather than the titlebar's brush directly: the titlebar may be a HostBackdrop
        // acrylic, which renders correctly only at the window edge and would leave sidebar
        ...
        // sidebar's ActualTheme hasn't settled yet this synchronously, so reading it here would be
        // stale and leave the colors on the old palette (the ActualThemeChanged handler is the
        // settled-theme backstop).
        _RefreshNodeLabelBrushes(requestedTheme);
    }
