...
namespace winrt::TerminalApp::implementation
{
    ...
    HRESULT TerminalWindow::Initialize(HWND hwnd)
    {
        ...
        else if (std::vector<Axan::StartupSession> axanTree; !_hasCommandLineArguments && !(axanTree = Axan::LoadStartupTree(_settings)).empty())
        {
            ...
            // new-tab or persisted layout. The tree is GLOBAL (GlobalAppSettings.StartupSessions,
            // decision D19); each entry references the WT profile it spawns under, and
            // the load falls back to the default profile for entries with no/unresolved ref.
            // Explicit CLI args (_hasCommandLineArguments) still win: they short-circuit this
            // branch (the load never runs) and fall through to the _appArgs branch below.
            // See decision D19.
            //
            // M5/M6: each node also carries an optional label template and its place in the
            // hierarchy (parentIndex, in DFS spawn order). Split the flattened tree into the
            // spawn actions (-> SetStartupActions) and the index-aligned per-node metadata,
            // which the sidebar drains as the startup tabs are created
            // (TerminalPage::_AddMissingSessionNodes) — templates label each node and
            // parentIndex reparents it under the already-spawned parent node.
            std::vector<ActionAndArgs> axanActions;
            std::vector<winrt::hstring> axanTemplates;
            std::vector<int32_t> axanParents;
            std::vector<winrt::hstring> axanIconOverrides; // M13
            std::vector<winrt::hstring> axanIconColors; // M13
            std::vector<winrt::hstring> axanColorTargets; // M13
            std::vector<winrt::hstring> axanEntryIds; // #422
            axanActions.reserve(axanTree.size());
            axanTemplates.reserve(axanTree.size());
            axanParents.reserve(axanTree.size());
            axanIconOverrides.reserve(axanTree.size());
            axanIconColors.reserve(axanTree.size());
            axanColorTargets.reserve(axanTree.size());
            axanEntryIds.reserve(axanTree.size());
            for (auto& session : axanTree)
            {
                axanActions.push_back(std::move(session.action));
                axanTemplates.push_back(std::move(session.labelTemplate));
                axanParents.push_back(session.parentIndex);
                axanIconOverrides.push_back(std::move(session.iconOverride));
                axanIconColors.push_back(std::move(session.iconColor));
                axanColorTargets.push_back(std::move(session.colorTarget));
    ...
    // axan: the two halves of the titlebar's centered title — the active
    // session's sidebar label, and its terminal title when that adds anything.
    hstring TerminalWindow::TitlebarPrimaryText()
    {
        return _root ? _root->TitlebarPrimaryText() : hstring{};
    }

    hstring TerminalWindow::TitlebarSecondaryText()
    {
        return _root ? _root->TitlebarSecondaryText() : hstring{};
    }
