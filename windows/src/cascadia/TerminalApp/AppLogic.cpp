...
// axan: auto-export each profile to a portable .toml on every settings change (D18).
...
namespace winrt::TerminalApp::implementation
{
    ...
    //     a background thread.
    // axan: refresh the portable .toml mirror after every successful settings load
    // (D18 — the Linux build auto-exports on UI change too). Reads the just-written
    // settings.json and writes <LocalState>\profiles\<uuid>.toml per profile plus
    // startup-sessions.toml for the global startup tree (D19), pruning mirrors of deleted
    // profiles. The settings watcher is non-recursive and matches only settings.json, so these
    // writes don't retrigger a reload.
    // Best-effort: any failure (e.g. an unpackaged process with no LocalState) is swallowed so it
    // can never break a settings reload.
    static void _autoExportProfileTomls()
    {
        try
        {
            const std::filesystem::path settingsPath{ std::wstring_view{ CascadiaSettings::SettingsPath() } };
            const auto localFolder = winrt::Windows::Storage::ApplicationData::Current().LocalFolder();
            const std::filesystem::path outDir = std::filesystem::path{ localFolder.Path().c_str() } / L"profiles";

            std::string error;
            const auto written = Axan::ProfileToml::ExportAllProfiles(settingsPath, outDir, error);
            if (written < 0)
            {
                Axan::Log::Warn("AppLogic", "auto-export: could not refresh the TOML mirror", { { "error", error } });
            }
            else
            {
                // A non-negative count can still carry a partial failure (a file that
                // wouldn't write, a stale mirror that wouldn't prune) — surface it.
                if (!error.empty())
                {
                    Axan::Log::Warn("AppLogic", "auto-export: TOML mirror refresh hit a failure", { { "error", error }, { "written", std::to_string(written) } });
                }
                Axan::Log::Info("AppLogic", "auto-export: refreshed portable profile TOML mirror", { { "count", std::to_string(written) }, { "dir", outDir.string() } });
            }
        }
        catch (...)
        {
            // ApplicationData throws for an unpackaged process; never let auto-export break a reload.
        }
    }
    ...
    void AppLogic::ReloadSettings()
    {
        ...
        // axan: keep the portable per-profile .toml mirror in sync with the live settings (D18).
        _autoExportProfileTomls();
