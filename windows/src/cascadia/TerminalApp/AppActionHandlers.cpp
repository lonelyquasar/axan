...
namespace winrt::TerminalApp::implementation
{
    ...
    // axan M8d: cycle the session sidebar (expanded / minimized / collapsed). Routed through
    // WT's action map (default Ctrl+Shift+B) so it preempts the focused terminal, unlike a
    // XAML accelerator. Ctrl+Shift+B avoids the tmux-prefix / readline Ctrl+B collision.
    ...
    // axan #429: move keyboard focus into the session sidebar (default Ctrl+Shift+Y). The
    // sidebar replaced the tab strip (D20) but no action ever moved focus off the terminal
    // and into the tree, so its built-in keyboard navigation was unreachable. Expands a
    // collapsed sidebar first; see _FocusSessionTree for the per-state focus targets.
    void TerminalPage::_HandleFocusSidebar(const IInspectable& /*sender*/,
                                           const ActionEventArgs& args)
    {
        _FocusSessionTree();
        args.Handled(true);
