...
namespace winrt::TerminalApp::implementation
{
    ...
    {
        // axan: the session-tree sidebar has replaced the tab strip, so the tabs
        // and the new-tab button are always hidden. The tab/pane infrastructure is
        // untouched (the sidebar still drives it); only the strip *visualization*
        // is retired. We deliberately keep the titlebar row present so the window's
        // drag region and min/max/close caption buttons — which live in the
        // NonClientIslandWindow's TitlebarControl as a sibling column of the tab
        // content, sized ~40px by the caption buttons — survive. Focus/fullscreen
        // still collapse the whole row (the window goes frameless there anyway).
        const auto rowVisible = !_isInFocusMode &&
        ...
        if (_tabView)
        {
            // axan: tabs are always collapsed now — the sidebar is the navigator.
            _tabView.Visibility(Visibility::Collapsed);
        }
        if (_newTabButton)
        {
            // axan: the "+" / new-tab dropdown belonged to the retired strip.
            _newTabButton.Visibility(Visibility::Collapsed);
        }
        if (_tabRow)
        {
            // Keep the titlebar row (caption buttons) except where focus/fullscreen
            // genuinely want the frameless view. NaN is XAML's "Auto" sizing value.
            _tabRow.Height(rowVisible ? NAN : 0);
        }
    }
                                   ...
                                   const int32_t suggestedNewTabIndex)
    {
        ...
        if (currentTabIndex != newTabIndex)
        {
            ...
            {
                // axan #428: suppress the session-tree reconcile across the transient
                // remove+insert of the same tab — see _OnSessionsCollectionChanged. A tab
                // genuinely LEAVING the window (_MoveTab's cross-window branch) goes through
                // _RemoveTab instead, where the reconcile still runs and prunes its node.
                _suppressSessionTreeReconcile = true;
                const auto restoreReconcile = wil::scope_exit([&]() noexcept { _suppressSessionTreeReconcile = false; });
                                         ...
                                         const IInspectable& /*eventArgs*/)
    {
        ...
        if (from.has_value() && to.has_value() && to != from)
        {
            try
            {
                ...
                // axan #428: same transient remove+insert of the same tab as _TryMoveTab —
                // suppress the session-tree reconcile so the reorder can't destroy the node.
                _suppressSessionTreeReconcile = true;
                const auto restoreReconcile = wil::scope_exit([&]() noexcept { _suppressSessionTreeReconcile = false; });
