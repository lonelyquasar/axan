...
namespace winrt::TerminalApp::implementation
{
    ...
    // axan: render the active session's name in the titlebar itself (primary =
    // session label, secondary = terminal title). The OS caption (SetWindowTextW)
    // still carries the plain title for the taskbar / alt-tab.
    void TitlebarControl::SetTitleText(const winrt::hstring& primary, const winrt::hstring& secondary)
    {
        TitlePrimary().Text(primary);
        TitleSecondary().Text(secondary);
        TitleSecondary().Visibility(secondary.empty() ? Visibility::Collapsed : Visibility::Visible);
        TitleTextRoot().Visibility((primary.empty() && secondary.empty()) ? Visibility::Collapsed : Visibility::Visible);
    }
    ...
    void TitlebarControl::_backgroundChanged(winrt::Windows::UI::Xaml::Media::Brush brush)
    {
        ...
        // axan: keep the title text contrasting with the pushed titlebar brush too.
        TitleTextRoot().RequestedTheme(theme);
    }
