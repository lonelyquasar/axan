...
#include "StartupSessions.h" // axan D19: global startup-sessions page
...
static const std::wstring_view startupSessionsTag{ L"StartupSessions_Nav" }; // axan D19
    ...
    void MainPage::_SetupProfileEventHandling(const Editor::ProfileViewModel profile)
    {
        ...
        _profileViewModelChangedRevoker = profile.PropertyChanged(winrt::auto_revoke, [=](auto&&, const PropertyChangedEventArgs& args) {
            ...
            if (settingName == L"CurrentPage")
            {
                ...
                // axan D19: the per-profile Startup sub-page is retired — the startup tree is now
                // global (the "Startup sessions" page). ProfileSubPage::Startup is no longer routed.
            }
        });
    }
