...
namespace winrt::Microsoft::Terminal::Settings::Editor::implementation
{
    ...
    LaunchEntryViewModel::LaunchEntryViewModel(hstring id, hstring parentId, hstring name, hstring directory, hstring command, hstring icon, hstring color, hstring colorTarget) :
        _Id{ id },
        _Icon{ icon },
        _ParentId{ parentId },
        _Name{ name },
    ...
    void LaunchEntryViewModel::Icon(const hstring& value)
    {
        if (_Icon != value)
        {
            _Icon = value;
            _NotifyChanges(L"Icon", L"IconPortable", L"IconPreview");
        }
    }

    // The editor-facing form of Icon: a builtin glyph (one raw Segoe PUA char, tofu in a
    // text box) reads back as its portable "builtin:NAME" token; typing such a token (or
    // an emoji / image path) stores the native form. An unrecognized builtin: name is
    // kept verbatim so the user can correct the typo — it's harmlessly inert as an icon.
    hstring LaunchEntryViewModel::IconPortable() const
    {
        bool mapped = false;
        return hstring{ Axan::IconRegistry::NativeToPortable(_Icon, mapped) };
    }

    void LaunchEntryViewModel::IconPortable(const hstring& value)
    {
        bool mapped = false;
    ...
    void LaunchEntryViewModel::Depth(int32_t value)
    {
        ...
        {
            _Depth = value;
            _NotifyChanges(L"Depth", L"IndentMargin");
        }
    }

    Windows::UI::Xaml::Thickness LaunchEntryViewModel::IndentMargin() const
    {
        // 16px per depth level — the Linux STARTUP_INDENT_STEP_PX.
        return Windows::UI::Xaml::Thickness{ static_cast<double>(_Depth) * 16.0, 0, 0, 0 };
    }

    Windows::UI::Xaml::Controls::IconElement LaunchEntryViewModel::IconPreview() const
    {
        // Render the M13-language icon id; an empty icon falls back to the neutral terminal
        // glyph (Segoe "CommandPrompt", the AxanIconRegistry "terminal" builtin) so an
