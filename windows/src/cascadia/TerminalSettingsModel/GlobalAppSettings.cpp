...
#include "LaunchEntry.h" // axan D19: get_self<LaunchEntry> for the StartupSessions deep copy
...
#undef GLOBAL_SETTINGS_COPY
    ...
    // axan D19: deep-copy the global startup tree so editing a settings clone never mutates
    // the source's LaunchEntry instances (the X-macro copy above is a shallow vector ref).
    if (_StartupSessions)
    {
        globals->_StartupSessions = winrt::single_threaded_vector<Model::LaunchEntry>();
        for (const auto& entry : *_StartupSessions)
        {
            globals->_StartupSessions->Append(get_self<LaunchEntry>(entry)->Copy());
        }
    }
