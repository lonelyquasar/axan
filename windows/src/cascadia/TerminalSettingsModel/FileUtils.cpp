...
static constexpr std::wstring_view UnpackagedSettingsFolderName{ L"axan\\" };
// axan package family name = sh.axan.Axan + the publisher hash derived from the cert
// subject "CN=Lonely Quasar". This constant is only consumed by GetReleaseSettingsPath()
// (the "migrate from the stable channel" path), which is compiled in ONLY under
// WT_BRANDING_PREVIEW/CANARY — axan builds WT_BRANDING_RELEASE, so it is currently dead.
// The live settings path is GetBaseSettingsPath(), which resolves the running package's
// own LocalState dynamically via KF_FLAG_FORCE_APP_DATA_REDIRECTION (no hardcoding).
static constexpr std::wstring_view ReleaseSettingsFolder{ L"Packages\\sh.axan.Axan_mdahwjbect15y\\LocalState\\" };
