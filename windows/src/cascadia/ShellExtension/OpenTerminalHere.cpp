...
{
    std::filesystem::path modulePath{ wil::GetModuleFileNameW<std::wstring>(wil::GetModuleInstanceHandle()) };
    // axan.exe,-101 will be the first icon group in the app exe.
    // We're using the windowed app (axan.exe) here explicitly, and not the launcher stub
    // (axan-cli, from GetWtExePath), because the app is the only one built with the icons.
    modulePath.replace_filename(L"axan.exe,-101");
