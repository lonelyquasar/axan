...
#pragma warning(suppress : 26490) // Don't use reinterpret_cast (type.1).
    ...
    // axan M11: the host's confirm dialog accepted — deliver the interrupt the guard
    // withheld. Clears the in-flight state so future Ctrl+C is evaluated afresh.
    void TermControl::ConfirmPendingInterrupt()
    {
        _interruptGuardPending = false;
        if (!_IsClosing())
        {
            _core.SendInterrupt();
        }
    ...
    // axan M11: the host's confirm dialog was dismissed — keep the typed line intact
    // and simply clear the in-flight state.
    void TermControl::CancelPendingInterrupt()
    {
        _interruptGuardPending = false;
    }
    ...
    bool TermControl::_KeyHandler(WORD vkey, WORD scanCode, ControlKeyStates modifiers, bool keyDown)
    {
        ...
        // axan M11: input-safety guard. A bare Ctrl+C (no Shift/Alt) with unsubmitted
        // input on the line would discard the typed line — the motivating bug
        // (rearchitecture.md §6). Intercept the keydown here, before it is translated
        // and sent to ConPTY, and ask the host to confirm. ConfirmPendingInterrupt /
        // CancelPendingInterrupt resolve the in-flight state. Ctrl+C with a clean line
        // (no pending input) is left untouched, so interrupting a running program is
        // unaffected. The guarded chord is fixed to Ctrl+C for now; a configurable
        // chord + user-facing toggle ride M14's Settings UI.
        if (keyDown &&
            !_interruptGuardPending &&
            vkey == 'C' &&
            modifiers.IsCtrlPressed() &&
            !modifiers.IsAltPressed() &&
            !modifiers.IsShiftPressed() &&
            _core.ShouldGuardInterrupt())
        {
            _interruptGuardPending = true;
            ShowCtrlCInputGuard.raise(*this, nullptr);
            return true;
        }
    ...
    {
        _refreshSearch();
        // axan M5: surface the settle signal to app-level consumers (the session-tree
        // sidebar recomputes templated labels here, once cwd/title have settled).
