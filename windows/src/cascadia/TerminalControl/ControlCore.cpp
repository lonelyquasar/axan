...
namespace winrt::Microsoft::Terminal::Control::implementation
{
                                    ...
                                    const ::Microsoft::Terminal::Core::ControlKeyStates modifiers)
    {
        ...
        // axan M11: maintain the "input since last newline" heuristic the Ctrl+C
        // guard reads. A submitted line (CR/LF) or an interrupt clears it; any
        // printable character arms it. Control chars (Ctrl+C, backspace, tab, …)
        // are intentionally ignored — coarse by design (rearchitecture.md §6).
        if (ch == L'\r' || ch == L'\n' || ch == L'\x3')
        {
            _hasUnsubmittedInput = false;
        }
        else if (ch >= L' ')
        {
            _hasUnsubmittedInput = true;
        }
    ...
    // axan M11: should a Ctrl+C be guarded right now? True only when the guard is
    // enabled (policy) AND there is unsubmitted input on the line (heuristic). The
    // frontend (TermControl) consults this on the Ctrl+C keydown and, if true,
    // confirms with the user before sending the interrupt instead of silently
    // discarding the typed line.
    bool ControlCore::ShouldGuardInterrupt() const
    {
        if (!_inputSafetyGuard || !_hasUnsubmittedInput)
        {
            return false;
        }

        // Don't guard while a full-screen TUI app owns the screen (alt buffer):
        // there Ctrl+C is the app's own chord and there is no shell line to lose.
        const auto lock = _terminal->LockForReading();
        return !_terminal->IsInAlternateScreenBuffer();
    }

    // axan M11: send the interrupt the guard withheld, once the user has confirmed.
    // Mirrors what an unguarded Ctrl+C keystroke delivers to a shell at the prompt:
    // a raw ETX (0x03) to the connection. Clears the pending-input heuristic since
    // the shell will discard the current line in response.
    void ControlCore::SendInterrupt()
    {
        _hasUnsubmittedInput = false;
        SendInput(L"\x03");
    }

    bool ControlCore::InputSafetyGuardEnabled() const noexcept
    {
        return _inputSafetyGuard;
    }

    void ControlCore::InputSafetyGuardEnabled(bool value) noexcept
    {
        _inputSafetyGuard = value;
    }
    ...
    void ControlCore::PasteText(const winrt::hstring& hstr)
    {
        ...
        // axan M11: a paste that doesn't end in a newline leaves an unsubmitted
        // command on the line, which the Ctrl+C guard should protect. Judge by the
        // raw pasted text (the filtered copy may carry bracketed-paste wrappers).
        const std::wstring_view raw{ hstr };
        _hasUnsubmittedInput = !raw.empty() && raw.back() != L'\r' && raw.back() != L'\n';
