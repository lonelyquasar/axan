...
// axan: WtExe is the execution-alias filename (what the user types: "axan" -> axan.exe),
// resolved under %LOCALAPPDATA%\Microsoft\WindowsApps\<package family name>\ for the
// packaged build. WindowsTerminalExe is the windowed app's filename, used only on the
// unpackaged fallback path (the app sitting next to the shell ext). WtdExe is vestigial:
// IsDevBuild() is false for the sh.axan.Axan identity under Release branding, so the
// WtdExe branch of GetWtExePath() is never taken.
constexpr std::wstring_view WtExe{ L"axan.exe" };
constexpr std::wstring_view WtdExe{ L"wtd.exe" };
constexpr std::wstring_view WindowsTerminalExe{ L"axan.exe" };
