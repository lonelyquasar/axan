                                       ...
                                       GVariant *options)
{
  ...
  /* Optional display-name template — set BEFORE add_screen so the sidebar's
   * initial label computation can see it. The template overrides the
   * OSC-title fallback chain; see terminal-sidebar.cc for variable
   * expansion semantics. */
  const char *name_template;
  if (g_variant_lookup (options, "axan-display-name-template", "&s", &name_template))
    terminal_screen_set_display_name_template (screen, name_template);

  /* Optional sidebar icon + per-node recolor — same ordering rationale as
   * name_template: set before add_screen so the first cell render picks them
   * up. set_appearance applies icon + color token + color_target in one shot;
   * absent options stay NULL (no icon / no color), which is correct for a
   * freshly created screen. The color token is resolved to a hex per active
   * theme at render time. */
  const char *icon = nullptr;
  const char *color = nullptr;
  const char *color_target = nullptr;
  g_variant_lookup (options, "axan-icon", "&s", &icon);
  g_variant_lookup (options, "axan-color", "&s", &color);
  g_variant_lookup (options, "axan-color-target", "&s", &color_target);
  if (icon != nullptr || color != nullptr || color_target != nullptr) {
    axan_log_debug ("sidebar.icons",
                    "create_instance appearance icon='%s' color='%s' target='%s'",
                    icon ? icon : "", color ? color : "", color_target ? color_target : "");
    terminal_screen_set_appearance (screen, icon, color, color_target);
  }

  terminal_window_add_screen (window, screen, -1);

  /* axan: sidebar-tree parent linkage. The client sets this option when a
   * profile's default-launch-entries declared this entry as a child of an
   * earlier entry — the forward-reference invariant guarantees the parent
   * screen is already present in the sidebar by the time we get here.
   * Failure to resolve (parent screen not found, would create a cycle) is
   * graceful: the row stays at root depth and the rest of startup
   * continues. We deliberately do NOT raise a DBus error for this since
   * the create_instance itself succeeded — partial hierarchy is acceptable. */
  const char *sidebar_parent_path;
  if (g_variant_lookup (options, "axan-sidebar-parent-screen", "&o", &sidebar_parent_path)) {
    TerminalScreen *sidebar_parent =
      terminal_app_get_screen_by_object_path (app, sidebar_parent_path);
    if (sidebar_parent != nullptr) {
      GtkWidget *mdi = terminal_window_get_mdi_container (window);
      if (TERMINAL_IS_SIDEBAR (mdi))
        terminal_sidebar_set_screen_parent (TERMINAL_SIDEBAR (mdi),
                                            screen, sidebar_parent);
    }
  }
...
/* CaptureWindowState — walk the most-recently-active axan window's sidebar
 * and return its tree as an a(ssssss) array suitable for writing into a
 * profile's default-launch-entries. Used by Preferences > Startup to
 * implement the "Capture current window" buttons.
 *
 * Failure mode: no axan windows visible → returns an empty array rather
 * than an error. The client can interpret empty as "nothing to capture"
 * and avoid clobbering the user's existing entries with nothing. */
static gboolean
terminal_factory_impl_capture_window_state (TerminalFactory *factory,
                                            GDBusMethodInvocation *invocation)
{
  TerminalApp *app = terminal_app_get ();
  GtkWindow *win = gtk_application_get_active_window (GTK_APPLICATION (app));
  if (win == nullptr) {
    /* Active window may be NULL when no window has focus; fall back to the
     * first axan window. Empty list → empty result. */
  ...
  GVariant *entries;
  if (win != nullptr && TERMINAL_IS_WINDOW (win)) {
    GtkWidget *mdi = terminal_window_get_mdi_container (TERMINAL_WINDOW (win));
    if (TERMINAL_IS_SIDEBAR (mdi)) {
      entries = terminal_sidebar_capture_entries (TERMINAL_SIDEBAR (mdi));
    } else {
      /* Not a sidebar (e.g., a non-axan window mistakenly in this app) —
       * empty array. */
      GVariantBuilder vb;
      g_variant_builder_init (&vb, G_VARIANT_TYPE ("a(ssssssss)"));
      entries = g_variant_builder_end (&vb);
    }
  } else {
    GVariantBuilder vb;
    g_variant_builder_init (&vb, G_VARIANT_TYPE ("a(ssssssss)"));
  ...
  terminal_factory_complete_capture_window_state (factory, invocation, entries);
  return TRUE; /* handled */
}
...
terminal_factory_impl_iface_init (TerminalFactoryIface *iface)
{
  ...
  iface->handle_capture_window_state = terminal_factory_impl_capture_window_state;
}
