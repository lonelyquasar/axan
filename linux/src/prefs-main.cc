...
/* State for the bus-name watcher that ties this Preferences process's
 * lifetime to the axan-server's. `has_appeared` distinguishes "server
 * vanished" from "server never started in the first place" — without it,
 * launching prefs while no server is running would call vanished
 * immediately and the dialog would close on its own. */
struct ServerWatchCtx {
  GApplication *app;
  gboolean has_appeared;
};

static void
prefs_server_name_appeared_cb (GDBusConnection *connection G_GNUC_UNUSED,
                               const char      *name G_GNUC_UNUSED,
                               const char      *name_owner G_GNUC_UNUSED,
                               gpointer         user_data)
{
  ServerWatchCtx *ctx = (ServerWatchCtx *) user_data;
  ctx->has_appeared = TRUE;
}

static void
prefs_server_name_vanished_cb (GDBusConnection *connection G_GNUC_UNUSED,
                               const char      *name G_GNUC_UNUSED,
                               gpointer         user_data)
{
  ServerWatchCtx *ctx = (ServerWatchCtx *) user_data;
  /* The watcher's first call is "vanished" if the name isn't owned at
   * watch-creation time — ignore that case so standalone-launched prefs
   * doesn't kill itself. Once the server has appeared at least once,
   * any subsequent vanish means it exited and we should close too. */
  if (!ctx->has_appeared)
    return;
...
#pragma GCC diagnostic pop
  ...
  g_set_prgname("axan-preferences");
  g_set_application_name(_("Axan Preferences"));
  ...
  /* Bind this preferences process's lifetime to the axan-server. When the
   * user closes the last terminal window the server exits its bus name;
   * without this watcher, prefs would linger as an orphan window. */
  ServerWatchCtx watch_ctx = { .app = app, .has_appeared = FALSE };
  guint watch_id = g_bus_watch_name (G_BUS_TYPE_SESSION,
                                     TERMINAL_APPLICATION_ID,
                                     G_BUS_NAME_WATCHER_FLAGS_NONE,
                                     prefs_server_name_appeared_cb,
                                     prefs_server_name_vanished_cb,
                                     &watch_ctx,
                                     nullptr);
