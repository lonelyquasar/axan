...
/* The window's minimum width in terminal columns. Upstream value was 4,
 * which let the window shrink to a few pixels and exposed the bash/readline
 * SIGWINCH-storm corruption during rapid window-resize wiggles: each
 * intermediate allocation crushed the terminal column below the prompt
 * width, bash redrew its prompt over the previous (now-too-narrow) one,
 * and the leftovers stayed in the buffer until something forced a clean
 * redraw. 24 is wide enough that a typical bash prompt
 * ("user@host:cwd$ ") fits in one line, so the storm can't accumulate.
 * Longer prompts (SSH-to-IPv6, multi-line PS1) can still trigger it; see
 * WATCHOUTS entry 3 for the proper-fix design (SIGWINCH debounce). */
#define MIN_WIDTH_CHARS 24
...
/* Cycle the sidebar through expanded → minimized → collapsed → expanded.
 * Wired from the toolbar chevron and the cycle-sidebar accelerator
 * (default <Control><Shift>b). Window also has an MDI container reference, so we
 * route through that — TerminalSidebar is the only TerminalMdiContainer
 * implementation today, so the cast is safe; if a second MDI ever lands
 * this becomes a no-op for it instead of a crash. */
static void
action_cycle_sidebar_cb (GSimpleAction *action G_GNUC_UNUSED,
                         GVariant *parameter G_GNUC_UNUSED,
                         gpointer user_data)
{
  TerminalWindow *window = (TerminalWindow*) user_data;
  TerminalWindowPrivate *priv = window->priv;
  if (priv->mdi_container == nullptr)
    return;
  if (!TERMINAL_IS_SIDEBAR (priv->mdi_container))
    return;
  terminal_sidebar_cycle_state (TERMINAL_SIDEBAR (priv->mdi_container));
}
...
/* axan: notebook action-box wiring is GtkNotebook-specific. The function is
 * preserved as a stub so its call site in `terminal_window_init` still links;
 * the call is now behind `if (GTK_IS_NOTEBOOK(...))` and so this body never
 * runs. Sidebar grows its own toolbar separately. */
static void
terminal_window_fill_notebook_action_box (TerminalWindow *window G_GNUC_UNUSED,
                                          gboolean add_new_tab_button G_GNUC_UNUSED)
{
}
...
terminal_window_init (TerminalWindow *window)
{
  const GActionEntry action_entries[] = {
    ...
    { "cycle-sidebar",       action_cycle_sidebar_cb,    nullptr,   nullptr, nullptr },
  ...
  /* axan: give the window a sensible startup size. GtkStack does not
   * propagate the active screen's natural width up the way GtkNotebook did,
   * so without this the window opens at scrollbar-overhead width on first
   * map. Default size only applies until the user resizes — geometry hints
   * (MIN_WIDTH_CHARS=4) still allow shrinking freely. */
  ...
  priv->mdi_container = TERMINAL_MDI_CONTAINER (terminal_sidebar_new ());
  ...
  /* axan: notebook action-box (new-tab + tabs-menu buttons) is GtkNotebook-
   * specific. The sidebar will grow its own toolbar later. */
  if (GTK_IS_NOTEBOOK (priv->mdi_container))
  ...
  g_snprintf (role, sizeof (role), "axan-window-%s", uuidstr);
...
terminal_window_update_size (TerminalWindow *window)
{
  ...
  /* axan: "resize the window to fit the active terminal's grid exactly" is a
   * notebook-era behavior. In sidebar mode the user's window size and paned
   * splitter position are authoritative, so we don't auto-fit on screen
   * switch / menubar toggle / zoom. (The terminal area still ends up the
   * right grid size because VTE handles grid recalculation on size-allocate.) */
  if (!GTK_IS_NOTEBOOK (priv->mdi_container))
    return;
