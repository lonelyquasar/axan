...
/* Display-name template controls the sidebar row label. Empty/NULL means
 * "fall back to OSC title or cwd basename"; non-empty means "the sidebar
 * should expand variables ($pwd, $user, $ssh-connection, etc) and use the
 * result, ignoring OSC titles." */
const char* terminal_screen_get_display_name_template (TerminalScreen *screen);
void        terminal_screen_set_display_name_template (TerminalScreen *screen,
                                                       const char *template_str);

/* Sidebar icon identifier (see default-launch-entries schema for forms).
 * NULL/empty = no explicit icon; the sidebar may auto-assign via hash or
 * fall through to the status-glyph placeholder. */
const char* terminal_screen_get_icon (TerminalScreen *screen);
void        terminal_screen_set_icon (TerminalScreen *screen,
                                      const char *icon);

/* Per-node recolor (M13 "Edit session node"). icon_color is "#RRGGBB"
 * (empty/NULL = theme foreground); color_target is "icon"/"text"/"both"
 * (empty/NULL = the "both" default). Mirrors the Windows per-node
 * iconColor/colorTarget. */
const char* terminal_screen_get_icon_color   (TerminalScreen *screen);
const char* terminal_screen_get_color_target (TerminalScreen *screen);
/* Set icon + recolor + recolor-target atomically; fires a single
 * icon-changed when any of the three actually changes. */
void        terminal_screen_set_appearance   (TerminalScreen *screen,
                                              const char *icon,
