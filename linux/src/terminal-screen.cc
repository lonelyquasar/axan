...
struct _TerminalScreenPrivate
{
  ...
  ExecData *exec_data;
  /* Optional display-name template for the sidebar row. Empty/NULL means
   * "use the default OSC-title fallback"; non-empty means the sidebar
   * should expand variables in this string and use the result. Set via
   * the create_instance DBus option `axan-display-name-template`. */
  char *display_name_template;
  /* Optional sidebar icon identifier. See default-launch-entries schema
   * for accepted forms (empty, `builtin:NAME`, abs path, bare filename).
   * Empty/NULL means "no explicit icon" — the sidebar may fall back to
   * the auto-assign hash (when enabled) or to the status-glyph
   * placeholder. Set via the create_instance DBus option `axan-icon`. */
  char *icon;
  /* Per-node recolor (the M13 "Edit session node" design handoff). Mirrors
   * the Windows frontend's per-node iconColor/colorTarget.
   *   icon_color   — "#RRGGBB" (or "#AARRGGBB"); empty/NULL = paint with the
   *                  theme foreground (the pre-color behavior).
   *   color_target — which surfaces the recolor paints: "icon", "text", or
   *                  "both"; empty/NULL is treated as the "both" default.
   * Session-only today, like the rename/icon overrides — set interactively
   * from the sidebar's Edit-session-node dialog, not persisted across a
   * restart (the persistent half rides the launch-entries schema work). */
  char *icon_color;
  char *color_target;
};
  ...
  switch (prop_id)
    {
      ...
      case PROP_TITLE:
        ...
        break;
      case PROP_DISPLAY_NAME_TEMPLATE:
        g_value_set_string (value, terminal_screen_get_display_name_template (screen));
        break;
      default:
  ...
  switch (prop_id)
    {
      case PROP_PROFILE:
        ...
        break;
      case PROP_DISPLAY_NAME_TEMPLATE:
        terminal_screen_set_display_name_template (screen, g_value_get_string (value));
        break;
      case PROP_TITLE:
        /* not writable */
      default:
...
terminal_screen_class_init (TerminalScreenClass *klass)
{
  ...
  /* Emitted whenever a per-node appearance field actually changes — the
   * icon (terminal_screen_set_icon) or the M13 recolor / recolor-target
   * (terminal_screen_set_appearance). No-op assignments don't fire.
   * Consumers — the sidebar's row renderer and #400's future dynamic
   * smart-match — connect here to know when to repaint the row's icon AND
   * label without polling. No payload: the handler reads back via
   * terminal_screen_get_icon / _get_icon_color / _get_color_target so it
   * always sees the post-change values even if the signal queues briefly. */
  signals[ICON_CHANGED] =
    g_signal_new (I_("icon-changed"),
                  G_OBJECT_CLASS_TYPE (object_class),
                  G_SIGNAL_RUN_LAST,
                  0,
                  nullptr, nullptr,
  ...
  g_object_class_install_property
    (object_class,
     PROP_DISPLAY_NAME_TEMPLATE,
     g_param_spec_string ("display-name-template", nullptr, nullptr,
                          nullptr,
                          GParamFlags(G_PARAM_READWRITE |
				      G_PARAM_STATIC_NAME |
				      G_PARAM_STATIC_NICK |
...
terminal_screen_finalize (GObject *object)
{
  ...
  g_free (priv->uuid);
  g_free (priv->display_name_template);
  g_free (priv->icon);
  g_free (priv->icon_color);
  g_free (priv->color_target);
...
const char *
terminal_screen_get_display_name_template (TerminalScreen *screen)
{
  g_return_val_if_fail (TERMINAL_IS_SCREEN (screen), nullptr);
  return screen->priv->display_name_template;
}

void
terminal_screen_set_display_name_template (TerminalScreen *screen, const char *template_str)
{
  g_return_if_fail (TERMINAL_IS_SCREEN (screen));
  /* Treat empty string the same as NULL — store NULL so consumers can do a
   * single non-null check. */
  if (template_str != nullptr && *template_str == '\0')
    template_str = nullptr;

  if (g_strcmp0 (screen->priv->display_name_template, template_str) == 0)
    return;

  g_free (screen->priv->display_name_template);
  screen->priv->display_name_template = g_strdup (template_str);
  g_object_notify (G_OBJECT (screen), "display-name-template");
}
...
void
terminal_screen_set_icon (TerminalScreen *screen, const char *icon)
{
  g_return_if_fail (TERMINAL_IS_SCREEN (screen));
  /* Normalize empty → NULL so consumers do one non-null check. */
  if (icon != nullptr && *icon == '\0')
    icon = nullptr;

  if (g_strcmp0 (screen->priv->icon, icon) == 0)
    return;

  g_free (screen->priv->icon);
  screen->priv->icon = g_strdup (icon);
  /* icon-changed (custom GSignal) rather than a notify on a GObject
   * property — the icon field has no GUI binding directly; consumers
   * (sidebar row renderer, eventual smart-match in #400) just want a
   * "refresh now" ping. Custom signal keeps the contract narrow and
   * avoids the boilerplate of a real property + g_object_get/set. */
  g_signal_emit (screen, signals[ICON_CHANGED], 0);
}

const char *
terminal_screen_get_icon_color (TerminalScreen *screen)
{
  g_return_val_if_fail (TERMINAL_IS_SCREEN (screen), nullptr);
...
const char *
terminal_screen_get_color_target (TerminalScreen *screen)
{
  g_return_val_if_fail (TERMINAL_IS_SCREEN (screen), nullptr);
  return screen->priv->color_target;
}

/* Set the per-node icon + recolor + recolor-target in one shot, firing a
 * single icon-changed only if something actually changed. icon-changed is
 * the sidebar's "refresh the appearance now" ping; the row renderer reads
 * all three fields back when it fires (so a color- or target-only change
 * still repaints the icon and the label). Empty strings normalize to NULL
 * so consumers do one non-null check; "both" is the color_target default
 * (callers may pass "" or NULL for it). */
void
terminal_screen_set_appearance (TerminalScreen *screen,
                                const char *icon,
                                const char *icon_color,
                                const char *color_target)
{
  g_return_if_fail (TERMINAL_IS_SCREEN (screen));
  TerminalScreenPrivate *priv = screen->priv;
  ...
  if (icon_color != nullptr && *icon_color == '\0')
    icon_color = nullptr;
  if (g_strcmp0 (priv->icon_color, icon_color) != 0) {
    g_free (priv->icon_color);
    priv->icon_color = g_strdup (icon_color);
    changed = TRUE;
  }

  /* Store "both" as NULL — it's the default, and consumers treat NULL as
   * "both" — so the field stays sparse and equality checks are cheap. */
  if (color_target != nullptr &&
      (*color_target == '\0' || g_str_equal (color_target, "both")))
    color_target = nullptr;
  if (g_strcmp0 (priv->color_target, color_target) != 0) {
    g_free (priv->color_target);
    priv->color_target = g_strdup (color_target);
    changed = TRUE;
  }
  ...
  *exec_argv_p = exec_argv = nullptr;
  ...
  if (argv)
  ...
  else if (as_shell)
    {
      ...
      /* Sized for the worst case: argv0path + argv0name + --rcfile + path
       * + NULL. Trailing slots are NULL when shell integration isn't
       * injected; downstream code is NULL-terminated, so unused slots are
       * harmless. */
      exec_argv = g_new0 (char*, 5);

      exec_argv[argc++] = shell;

      bool const login_shell =
        g_settings_get_boolean (profile, TERMINAL_PROFILE_LOGIN_SHELL_KEY);
      if (login_shell)
        exec_argv[argc++] = g_strconcat ("-", only_name, nullptr);
      else
        exec_argv[argc++] = g_strdup (only_name);

      /* Inject our bash rcfile for non-login interactive bash so VTE OSC 7
       * (cwd reporting) and OSC 133 (prompt marks) install. /etc/profile.d
       * is only sourced for login shells; without this, features like
       * right-click Copy Path silently break. Login bash already gets the
       * hooks via the normal profile chain. See lonelyquasar/axan#405. */
      GSettings *global_settings =
        terminal_app_get_global_settings (terminal_app_get ());
      bool const integration_enabled =
        g_settings_get_boolean (global_settings,
                                TERMINAL_SETTING_SHELL_INTEGRATION_KEY);
      if (!login_shell && integration_enabled && g_str_equal (only_name, "bash")) {
        exec_argv[argc++] = g_strdup ("--rcfile");
        exec_argv[argc++] = g_build_filename (TERM_PKGDATADIR,
                                              "shell-integration",
                                              "bash",
                                              "axan-rcfile.sh",
                                              nullptr);
