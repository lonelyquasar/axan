...
{
  /* Upstream gnome-terminal held the server app alive while a prefs
   * subprocess existed (g_application_hold/release across init/finalize)
   * so prefs could keep talking to the server. That created a deadlock
   * once the user closed the last terminal window: server stayed alive
   * because prefs held it, prefs stayed alive because nothing told it
   * to close. We drop the hold so the server's normal lifecycle wins —
   * the last terminal window closing lets the server's inactivity
   * timeout fire, the private bus to prefs closes, and prefs' existing
   * connection-closed handler quits the prefs process. Net effect:
   * closing axan closes prefs too. */
}
