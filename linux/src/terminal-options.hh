...
typedef struct
{
  ...
  /* axan: profile/settings/keybindings import/export (see #401). When any
   * is set, terminal.cc takes an early-exit path before D-Bus activation —
   * the operation runs against dconf directly, no axan-server needed.
   * The boolean variants don't take an argument (they operate on the
   * canonical $XDG_CONFIG_HOME/axan path), the *_path/*_uuid variants do. */
  char    *export_profile_uuid;
  char    *import_profile_path;
  char    *export_output_path;
  gboolean export_settings;
  char    *import_settings_path;
  gboolean export_keybindings;
  char    *import_keybindings_path;
  gboolean export_all;
  char    *import_all_path;
  gboolean import_icons;
...
typedef struct
{
  ...
  guint active : 1;
  guint wait : 1;
  /* Sidebar hierarchy linkage, populated only when expanded from a profile's
   * default-launch-entries. -1 means "this tab is a root in the sidebar tree"
   * (the normal case). Otherwise the value is the zero-based index, within
   * this tab list, of the InitialTab that should be the parent in the
   * sidebar. Forward references only: parent_tab_index < this tab's own
   * position. Honored by the spawn path in terminal.cc which maps each
   * created screen's object path back into the hierarchy. */
  int parent_tab_index;
  /* Display-name template for the sidebar row, also populated only via the
   * profile expansion path. Empty/NULL means "use the default OSC-title
   * fallback chain"; non-empty means "expand $pwd/$user/etc and use the
   * result as the row label, ignoring OSC titles." The template string is
   * owned by the InitialTab and freed in initial_tab_free. */
  char *name_template;
  /* Icon identifier for the sidebar cell (collapsed + optionally expanded).
   * See the schema description on default-launch-entries for accepted forms.
   * NULL/empty means "no explicit icon" — the cell falls back to the status
   * glyph placeholder. Currently not wired to the spawn path; reserved for
   * the icon-rendering pieces. Owned by the InitialTab. */
  char *icon;
  /* Per-node recolor, populated only via the profile expansion path. `color`
   * is the token (palette name like "blue", or a literal "#RRGGBB"; empty =
   * theme foreground); `color_target` is "icon"/"text"/"both" (empty = both).
   * Carried to the server as the axan-color / axan-color-target CreateInstance
   * options. Owned by the InitialTab. */
  char *color;
  char *color_target;
} InitialTab;
...
void terminal_options_expand_profile_launch_entries (TerminalOptions *options);
