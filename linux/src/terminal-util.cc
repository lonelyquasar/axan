...
{
  /* Upstream gnome-terminal pointed at help:gnome-terminal — a Yelp-format
   * URI that routes through GNOME's installed help docs. We don't ship
   * those for axan; the user-facing reference lives in the project's
   * README. `topic` distinguishes "general" from "pref" (Preferences →
   * Help calls this with "pref"); both currently resolve to the same
   * README because there isn't enough surface to warrant separate pages.
   * If/when we add longer-form docs they can branch on topic here. */
  const char *url = "https://github.com/lonelyquasar/axan#readme";
  (void) topic;

  gs_free_error GError *error = nullptr;
  if (!open_url (nullptr, url, gtk_get_current_event_time (), &error))
    {
...
#define ABOUT_URL "https://github.com/lonelyquasar/axan"
...
terminal_util_show_about (void)
{
  ...
  comment = g_strdup_printf("%s\n%s %s",
                            _("A terminal with a tree-of-shells sidebar, forked from GNOME Terminal"),
                            vte_version,
                            vte_get_features ());
...
{
  const gchar *license[] = {
    N_("axan is free software: you can redistribute it and/or modify "
       "it under the terms of the GNU General Public License as published by "
       "the Free Software Foundation, either version 3 of the License, or "
       "(at your option) any later version."),
    N_("axan is distributed in the hope that it will be useful, "
       "but WITHOUT ANY WARRANTY; without even the implied warranty of "
       "MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the "
       "GNU General Public License for more details."),
...
#define TERMINAL_CACHE_DIR                 "axan"
