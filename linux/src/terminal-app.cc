...
/* Both the server and the preferences process drive the libhandy color
 * scheme (the prefs window must follow the same light/dark choice), so the
 * libhandy header is included unconditionally rather than server-only. */
#include <handy.h>
...
#define AXAN_PREFERENCES_ICON_NAME    "sh.axan.Axan.Preferences"
...
struct _TerminalApp
{
  ...
  /* The style manager drives the light/dark color scheme and is needed by
   * both the server windows and the preferences window, so it lives outside
   * the TERMINAL_SERVER-only block above. */
...
/* Applies the theme-variant setting to the libhandy color scheme (or, on
 * systems without color-scheme support, the GTK prefer-dark property). Built
 * for both the server and the preferences process so the prefs window follows
 * the same light/dark choice as the terminal windows. */
static void
                      ...
                      GVariant *target /* floating, consumed */)
{
  ...
  /* Truncate at MAX_CHARS UTF-8 code points so a runaway profile name doesn't
   * blow the popover menu out horizontally (axan#406). The numeric prefix
   * still disambiguates same-prefix names. */
  const int MAX_CHARS = 40;
  const char *truncate_at = nullptr;
  glong char_count = 0;
  for (const char *q = label; *q != '\0'; q = g_utf8_next_char (q)) {
  ...
  for (p = label; *p && p != truncate_at; p++) {
    if (*p == '_')
      g_string_append (str, "__");
    else
      g_string_append_c (str, *p);
  }
  if (truncate_at != nullptr)
    g_string_append (str, "…");
...
terminal_app_startup (GApplication *application)
{
  ...
  gdk_set_program_class("Axan");
#elif defined(TERMINAL_PREFERENCES)
  gdk_set_program_class("Axan-preferences");
#else
#error
#endif
...
{
  /* libhandy must be initialised in every process that uses its widgets or
   * style manager. The preferences window needs both, so this is not gated
   * on TERMINAL_SERVER. */
  hdy_init ();
...
#if defined(TERMINAL_SERVER)
  gtk_window_set_default_icon_name (AXAN_ICON_NAME);
#elif defined(TERMINAL_PREFERENCES)
  gtk_window_set_default_icon_name(AXAN_PREFERENCES_ICON_NAME);
#else
#error
#endif
  ...
  /* Theme variant applies to both the terminal windows and the preferences
   * window — kept out of the TERMINAL_SERVER guard so the prefs process
   * follows the same light/dark choice instead of defaulting to light. */
  ...
  /* First-run sweetener: rename the default profile from "Unnamed" to the
   * user's real name (or login name, if GECOS is empty). We touch
   * visible-name only when no user value has ever been written — i.e., the
   * schema's "Unnamed" default is still in effect. After that, anything the
   * user types is final, including "Unnamed" if they want it back.
   *
   * Limitation: applies only to the default profile. New profiles created
   * via the prefs UI still start at "Unnamed" — extend this logic to
   * terminal_settings_list_add_child if/when we want the same default for
   * all new profiles, not just the bootstrap one. */
  {
    gs_unref_object GSettings *default_profile =
      terminal_settings_list_ref_default_child (app->profiles_list);
    if (default_profile != nullptr) {
      gs_unref_variant GVariant *user_value =
        g_settings_get_user_value (default_profile, "visible-name");
      if (user_value == nullptr) {
        const char *real = g_get_real_name ();
        if (real == nullptr || *real == '\0' || g_str_equal (real, "Unknown"))
          real = g_get_user_name ();
        if (real != nullptr && *real != '\0')
          g_settings_set_string (default_profile, "visible-name", real);
      }
    }
  }
...
#ifdef TERMINAL_SERVER
  /* TOML mirrors of dconf settings — auto-rewritten on any change so the
   * TOML files under ~/.config/axan always reflect current state. The prefs
   * UI "Import Settings from Config" button reads back from those files. */
  axan_auto_export_init (app);
#endif
}
 ...
 * @app: a #TerminalApp
 *
 * Returns: (tranfer none): the cached #GSettings object for the sh.axan.Axan.Preferences schema
 */
GSettings *
